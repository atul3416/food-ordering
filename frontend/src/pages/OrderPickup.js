...
const OrderPickup = () => {
    ...
    useEffect(() => {
        ...
        fetch('http://127.0.0.1:8000/api/order-pickup/')
            .then(res => res.json())
            .then(data => {
                setOrders(data)
            })
    ...
    return (
        <AdminLayout>
            <div>
                <h3 className='text-center text-primary mb-4'><i className='fas fa-list-alt me-1'></i>Detail of order Pickedup</h3>
                <h5 className='text-end text-mutate'>
                    <i className='fas fa-database me-1'></i>Total Pickuped Orders:
